# 문제해결 순서
# 1. 주어진 예제의 답이 어떻게 나오는지 확실하게 계산해본다.
# 2. 알고리즘을 떠올리고, 시간복잡도를 계산합니다.
# 3. 시간 안에 나오는 것 같다면, 코드를 작성합니다. 
# 4. 예제를 넣어봅니다. 안나오면 고칩니다.
# 5. 예제가 나왔는데도 만점이 아니라면 

# 5-1 답이틀린경우 : n=1 n = max 와 같은 경계에 걸린 경우를 넣어본다.
# 5-2 런타임 에러 : index 체크
# 5-3 시간복잡도 : 더 좋은 솔루션 생각 -> 삼성의 경우 무한루프일 가능성이 큼
# 5-4 메모리 초과 : 메모리는 줄일 생각 -> 삼성의 경우는 만날 가능성 없음
# 5-5 컴파일 에러 : 에러내용 수정
